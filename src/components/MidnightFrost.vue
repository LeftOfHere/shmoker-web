...
const sparkleContainer = ref();

let sparkling = false;

const sparkleDelay = 2;

let children: any = [];

onMounted(() => {
  for (let n = 0; n < 100; n++) {
    let size = n % 2 === 0 ? "small" : n % 3 === 0 ? "medium" : "large";

    const el = document.createElement("div");

    el.id = `star-${n}`;
    el.classList.add(size, "shine");

    el.style.setProperty("top", Math.random() * 100 + "%");
    el.style.setProperty("left", Math.random() * 100 + "%");
    el.style.setProperty("animation-delay", Math.random() * sparkleDelay + "s");

    el.style.setProperty("opacity", Math.random() < 0.8 ? "0" : "1");

    children.push(el);
    sparkleContainer.value.append(el);
  }
});
...
function hoverLeave() {
  ...
  //     sparkleContainer.value.removeChild(child);
  //   }
  sparkling = false;
}
</script>
...
/* .shine:hover {
  animation: glitter 1s linear 0s infinite normal;
  -webkit-animation: glitter 1s linear 0s infinite normal;
  -moz-animation: glitter 1s linear 0s infinite normal;
  -ms-animation: glitter 1s linear 0s infinite normal;
  -o-animation: glitter 1s linear 0s infinite normal;
} */
